...
extension Event {
    ...
    static func all(forSport sportID: Sport.ID, isInPlay: Bool?, on request: Request) async throws -> [Event] {
        try await request.eventService.events(forSport: sportID, isInPlay: isInPlay, locale: request.locale)
            .map(Event.init)
    }

    static func all(forCompetition competitionID: Competition.ID, on request: Request) async throws -> [Event] {
        try await request.eventService.events(forCompetition: competitionID, locale: request.locale)
            .map(Event.init)
    }

    static func find(_ id: Event.ID, on request: Request) async throws -> Event? {
        try await request.eventService.event(withID: id, locale: request.locale)
            .map(Event.init)
    }

    static func find(forMarket marketID: Market.ID, on request: Request) async throws -> Event? {
        try await request.eventService.event(forMarket: marketID, locale: request.locale)
            .map(Event.init)
    }
