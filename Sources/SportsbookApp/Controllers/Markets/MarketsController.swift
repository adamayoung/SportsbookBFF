...
public struct MarketsController: RouteCollection {
    ...
    func indexForEvents(request: Request) throws -> EventLoopFuture<JSONAPIModel<[Market]>> {
        guard let eventID = request.parameters.get("eventID", as: Int.self) else {
            throw Abort(.internalServerError)
        }

        return Market.all(forEvent: eventID, on: request)
    ...
    func show(request: Request) throws -> EventLoopFuture<JSONAPIModel<Market>> {
        guard let id = request.parameters.get("marketID") else {
            throw Abort(.internalServerError)
        }

        return Market.find(id, on: request)
