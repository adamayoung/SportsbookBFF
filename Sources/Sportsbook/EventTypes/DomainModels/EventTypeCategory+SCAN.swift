...
extension EventTypeCategory {

    init?(eventTypeID: EventTypeDomainModel.ID) {
        guard let category = (EventTypeCategory.allCases.first { $0.eventTypes.contains(eventTypeID) }) else {
            return nil
        }
...
private extension EventTypeCategory {
